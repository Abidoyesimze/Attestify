...
describe("AttestifyVault", async () => {
  ...
  const ONE = 10n ** 18n;
  const VAULT_CONTRACT = "contracts/AttestifyVault.sol:AttestifyVault";
  ...
  async function deployFixture() {
    ...
    const vaultImpl = await viem.deployContract(VAULT_CONTRACT);
    ...
    const vault = await viem.getContractAt(VAULT_CONTRACT, proxy.address);
